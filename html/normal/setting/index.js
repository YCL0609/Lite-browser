...
async function loadSettings() {
    ...
    if (mainWin.background) litebrowser.getFile(mainWin.background, 'base64')
        .then((base64) => {
            const ext = currentSettings.mainWin.background.split('.').pop();
            const mime = 'image/' + (ext === 'svg' ? 'svg+xml' : ext);
            document.getElementById('preview').src = `data:${mime};base64,${base64}`;
        });
}
